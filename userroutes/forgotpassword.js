...
const forgotPassword = async (User, jwt, nodemailer, req, res) => {
  ...
  try {
      ...
      const mailOptions = {
          ...
          html: `
              <div style="font-family: Arial, sans-serif; padding: 20px;">
                  ...
                  <a href="https://api.zap70.com/forget-password/${user._id}/${token}" style="display: inline-block; padding: 10px 20px; background-color: #007bff; color: #fff; text-decoration: none; border-radius: 5px; font-size: 16px;">Reset Password</a>
      ...
      transporter.sendMail(mailOptions, (error, info) => {
          if (error) {
              console.log(error);
              return res.status(500).json({ error: true, message: "Error Sending Mail" });
          } else {
      ...
      console.log(error);
      return res.status(500).json({ error: true, message: "Error Sending Mail", errorMessage: error.message });
  }
};
